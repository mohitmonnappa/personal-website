...
// Hand-drawn squiggle variants, cycled through (with one random starting
// point) on hover, same idea as Osmo's "draw random underline" effect —
// reimplemented with Framer Motion's pathLength/pathOffset instead of
// GSAP's (paid, club-only) DrawSVGPlugin, to match this site's existing
// motion library and avoid a new dependency.
...
export function Nav() {
  ...
  const [variantByHref, setVariantByHref] = useState<Record<string, number>>(
    {}
  );
  const nextVariant = useRef<number | null>(null);
        ...
        <div className="flex h-16 items-center justify-between">
          ...
          <nav className="hidden items-center gap-8 sm:flex">
            {LINKS.map((link) => {
              ...
              const showDraw = hoveredHref === link.href && !active;
              const variant = UNDERLINE_PATHS[variantByHref[link.href] ?? 0];
              return (
                <Link
                  key={link.href}
                  href={link.href}
                  ...
                  onMouseLeave={() =>
                    setHoveredHref((h) => (h === link.href ? null : h))
                  }
                  className={clsx(
                ...
                >
                  ...
                  <span className="pointer-events-none absolute -bottom-1.5 left-0 right-0 h-2 text-clay">
                    <AnimatePresence>
                      {showDraw && (
                        <motion.svg
                          viewBox="0 0 310 40"
